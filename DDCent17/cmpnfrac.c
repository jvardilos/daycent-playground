...
/*          *frac_nh4 = *ammonium/(minerl[(iel)*CMXLYR + (*clyr-1)]); */
      ...
      if ((*frac_nh4 > 1.0 + 1.0E-04) || (*frac_nh4 < 0.0 - 1.0E-04)) {
        fprintf(stdout, "Error in cmpnfrac, frac_nh4 = %12.10f\n", *frac_nh4);
        fprintf(stdout, "clyr = %1d\n", *clyr);
        fprintf(stdout, "ammonium = %12.10f\n", *ammonium);
        fprintf(stdout, "Nsum = %12.10f\n", Nsum);
        fprintf(stdout, "minerl[%1d][1] = %12.10f\n", *clyr, minerl[(iel)*CMXLYR+(*clyr-1)]);
        abortmssg("Fraction NH4 out of bounds in cmpnfrac");
      }
