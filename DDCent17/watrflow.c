...
**            KLK 28Sept11   Diverted err to message output
**            KLK 20Feb14    Added methane changes
**
**  HISTORY:
**     5/ 4/15  KLK  eliminate swctemp. pass setasmos to eliminate an error
**                   in the deep storage layer
...
**                 and litter (cm H2O)
**    inipswc    - initial profile swc for balance (cm H20)
...
**                     This replaces the sitepar array
...
**    sitepar->sradadj[]    - srad adjust for cloud cover & transmission coeff 100(1-cldcov)
...
**                            2 = read srad
**                            3 = use extra drivers PET and read srad
...
**                    changed the calculation to match that used in hwdrain
...
#include "thawpulse.h"
    ...
    void watrflow(int *jday, int *month, int *nlayer, int *nlaypg,
                  ...
                  float *watr2sat, double *srad, float *sradadj, double *inipswc)
    {
      ...
      *pottransp = 0.0f;
      ...
      if (flags->debug > 1) {
        printf("Entering function watrflow    %15.7f\n",*inipswc);fflush(stdout);
      }
...
/*      printf("pet in watrflow = %8.4f\n", *pet); */
      ...
      swpavg = trwtavg(nlaypg, layers, flags);
...
!!*/
      if (sitepar->usexdrvrs == 1 || sitepar->usexdrvrs == 3) {
        snowmodel(*jday, *tempmax, *tempmin, *avgtemp, -1.0f, *pptactual,
                  &pptsoil, *solrad, *rhumid, *windsp, *sradadj, /* sitepar->cldcov[*month], */
                  sitepar->rlatitude, sitepar->albedo, snowpack, melt,
                  accum, sublim, &petleft, sitepar->sublimscale, petleft,
                  tmelt, snlq, *srad, *daylength);
      } else {
      ...
      if (petleft < 0.0) {
        sprintf(msgbfr, "PET/AET balance failure in watrflow.  petleft = %12.10f",petleft);
        abortmssg(msgbfr);
      }
      ...
      *intrcpt = wintstcr + wintlit;
      ...
      if (*snowpack > 0) {
        ...
        potbse(&bserate, totagb, fbse, petleft, layers, *biolive);
      }
      ...
      *pottransp = bstrate;
      /* Limit transpiration as a result of root resistance to water flow */
      /* cak - 10/13/2009
       ****** Removed this equation.
              It simplifies to *pottransp = 0.35617 + .0106*petleft
              Essentially it caps the potential transpiration used for the
              HI water stress at 0.36+ (for petleft = 1  pottransp = 0.367)
              This produces HI > HIMAX when bstrate > 0.36  KLK 18 Nov 2012
      if (*pottransp > 0.36) {
        *pottransp = (0.42f - 0.36f) / (6.0f - 0.36f) * (petleft - 6.0f) + 0.42f;
      }
*/
      ...
      soiltemp(*jday, *tempmin, *tempmax, layers->depth, layers->width,
               ...
               layers->orgfrac, layers->lyblkd, layers->swc, layers->numlyrs,
      ...
      h2oflux(*jday, layers->numlyrs, layers->swc, layers->swcmin,
              ...
              sitepar->hours_rain, sitepar->dmpflux, &aetdly, inipswc,
              outflow, wfluxout, *snowpack, runoffdly, basef, baseflow,
              *watertable, sitepar->hpotdeep, sitepar->ksatdeep);

      chkthwpulse(*snowpack, *time, *jday, *melt, soil->soiltavg);
...
/*      printf("petleft = %8.4f\n", petleft); */
      ...
      if (petleft > 0) {
        ...
        /* SHouldn't this be the root limited transpiration?
        soiltransp(transp, tranlayers, *pottransp, layers, &aetdly); */
...
/*      *stormflow = wfluxout[layers->numlyrs-1] * (*stormf); */
      ...
      *watr2sat = 0.0f;
      if ((*watertable == 1) && (*watrflag == 1)) {
        for (ilyr=0; ilyr < layers->numlyrs; ilyr++) {
/*        was
          swcsat = (1.0f - layers->lyblkd[ilyr] / (float)PARTDENS) *
                   layers->width[ilyr];
          wtosat = swcsat - (float)layers->swc[ilyr];
          use the definition from hwdrain directly in wtosat  KLK 8Sept14 */
          wtosat = (layers->thetas_bd[ilyr] * layers->width[ilyr]) -
                   (float)layers->swc[ilyr];
          if (wtosat > 0.0) {
      ...
      setasmos(asmos, nlayer, layers->swc, &layers->numlyrs, avh2o, rwcf);
