...
**    impedance      - flag, 1 if the layer is frozen or otherwise impermeable
...
**    inipswc        - daily initial cumulative water in the profile (cm)
...
**    rainDay        - UNUSED Removed 6/24/2014 KLK
**                     This was always drainDay - sitepar->drainlag
...
/*      dt_hours = 2; */
      ...
      if (debug) {
        printf("Day %1d: watrinput (cm) = %12.10f %12.10f\n", jday, watrinput, *inipswc);fflush(stdout);
      }
      ...
      *soilEvap = 0.0f;
      ...
      if (((infil_time % dt_hours) != 0) || (infil_time > HOURS_PER_DAY)) {
        sprintf(msgbfr,"hours_rain_param must be a multiple of %1d and < %1d", dt_hours, HOURS_PER_DAY);
        abortmssg(msgbfr);
      }
      ...
      if (watrinput > 0.0) {
        ...
        /* the deep storage layer at the bottom of the soil profile, swc[numlyrs] */
      ...
      while ((fabs(cumtime - SEC_PER_DAY) > 0.01) && (!restore)) {
        ...
        if (watertable == 1) {
          ...
          /* If we are not simulating a water table the only direction water
             can flow from the deep storage layer is out of the bottom */
...
/*          evapRate = 0.0; */
        ...
        for (ilyr=0; ilyr < numlyrs; ilyr++) {
          ...
          /* thetas changed to thetas_bd.  -cindyk 9/18/00 */
/*          swcsat = layers->thetas[ilyr]*width[ilyr]; */
          swcsat = layers->thetas_bd[ilyr]*width[ilyr];
          ...
          /* net flux out of ilyr */
            ...
            /* net flux must be reduced so soil does not get below the minimum swc */
            if (swc[ilyr] + 0.0000001 < swcmin[ilyr]) {
              printf("Day: %3d, h2oflux  layer %2d swc = %1f < swcmin = %1f\n", jday,
                     ilyr, swc[ilyr],swcmin[ilyr]);
            }
            if (debug) {
...
/*            flux[ilyr+1] = (swc[ilyr] - swcmin[ilyr] + flux[ilyr]*dt)/dt;
          ...
          if (swc[ilyr] + 0.0000001 < swcmin[ilyr]) {
            printf("\nDay: %1d, layer %1d too dry, swc = %10.8f flux[%1d] = %10.8f  flux[%1d] = %10.8f",
                   jday,ilyr,swc[ilyr],ilyr,dt*flux[ilyr], ilyr+1, dt*flux[ilyr+1]);
            sprintf(msgbfr,"layer %1d too dry, swc = %10.8f on day %1d", ilyr, swc[ilyr], jday);
            abortmssg(msgbfr);
          }
...
/*          if (flux[numlyrs]*dt < -1.0E-9) { */
      ...
      if (!restore) {
        /* Check for error conditions */
        if (fabs(tdiff) > 0.01) {
          sprintf(msgbfr,"bad time step in h2oflux  tdiff = %12.10f\n", tdiff);
          abortmssg(msgbfr);
        }
      }
      ...
      *runoffdly = (float)cumrnf;
      ...
      if (watertable != 1) {
        balans = (*inipswc - cumwtr) + cuminfl - cumevap - *baseflow;

        if (fabs(balans) > 5.0E-5) {
          printf("ERROR: h2oflux water balance on day %1d.%1d:  balans = %12.10f\n", jday, tstep_cnt, balans);
          printf("  watrinput = %12.10f\n", watrinput);
          printf("  cumrnf    = %12.10f\n", cumrnf);
          printf("  cuminfl   = %12.10f\n", cuminfl);
          printf("  inipswc   = %12.10f\n", *inipswc);
          printf("  cumwtr    = %12.10f\n", cumwtr);
          printf("  cumevap   = %12.10f\n", cumevap);
          printf("  baseflow  = %12.10f\n", *baseflow);
          printf("  cumdrn    = %12.10f\n", cumdrn);
          printf("restore = %1d\n", restore);
          abortmssg("water balance failed in h2oflux");
        } else {
