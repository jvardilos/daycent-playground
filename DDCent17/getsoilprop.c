...
#define VERYFINE 4              /* texture type for volcanic soil */
    ...
    {
    /* saturation soil hydraulic `conductivity' by soil texture class (m/s)  (unused)
      ...
      if ((sand > 100 || sand < 0) || (silt > 100 || silt < 0) ||
          (clay > 100 || clay < 0)) {
        sprintf(msgbfr, "Invalid texture in getsoilprop: sand = %f, silt = %f, clay = %f",
         sand, silt, clay);
        abortmssg(msgbfr);
      }
      ...
      if (*texture == VERYFINE) {
        ...
        sprintf(msgbfr, "Unknown texture type: sand = %f, silt = %f, clay = %f",
                 sand, silt, clay);
        abortmssg(msgbfr);
      }
