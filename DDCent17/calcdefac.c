...
extern void tcalc(float *, float [], float *);

    void calcdefac(int *texture, float *tfunc, float *bgwfunc, float *agdefac,
                   float *bgdefac, float *avgwfps, float teff[],
                   float *rprpet, int *idef, float *ppt, float *snow,
                   float *avgstemp)
    {
      int   ilyr;
      float a, b, c, d;
        ...
        case 3:
          ...
          switch (*texture) {
            ...
            case VERYFINE:
              ...
              break;
            default:
              sprintf(msgbfr,"unknown texture = %1d in calcdefac", *texture);
              abortmssg(msgbfr);
              exit(1); /* leave this; prevents undefined a, b, c, d, compiler warnings */
          }
          ...
          *bgwfunc = agwfunc;
          break;
        default:
          sprintf(msgbfr,"Error in calcdefac, unknown idef value = %1d", *idef);
          abortmssg(msgbfr);
      }
      ...
      *avgstemp = (soil->soiltavg[1] * layers->width[1] +
      ...
      /* cak - 04/01/04 */
      /*Modify the effect of precipitation on aboveground decomposition CAK 08/18/11*/
      if (*ppt > 0.5) {
