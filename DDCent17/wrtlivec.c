...
#include "soilwater.h"
    ...
    void wrtlivec(float *time, int *curday, float *aglivc, float *bglivcj,
                  float *bglivcm, float *rleavc, float *frootcj, float *frootcm,
                  float *fbrchc, float *rlwodc, float *crootc, float *frnutc)
    {
      ...
      fprintf(files->fp_livec, "%10.4f  %10.4f  %10.4f  %10.4f  %10.4f  %10.4f\n",
