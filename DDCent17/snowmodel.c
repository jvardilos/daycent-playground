...
**              to sradadj.  This is effectively clrsky (1-cldcov/100)
**    11/27/95 (MDH) Switch the order of melt and sublimation.
**               Sublimate first.
**
**  INPUTS:
...
**    solrad      - net incoming solar radiation (langleys/day)
**                  (changed local name netrad for traceability KLK)
...
float svapor(float);
    ...
    void snowmodel(int jday, float tmax, float tmin, float tavg,
                   float tmpcrit, float ppt, float *pptadj, float solrad,
                   float relhum, float windsp, float clrsky, float rlatitude,
...
*!!*/
      ...
      if (*snow > 0.0) {
        ...
        PET = pet_snow(jday, tavg, windsp, relhum, solrad,
                       *Emelt, albedo, rlatitude, clrsky);
        /* mm SWE sublimation */
        ...
        *Esublim = min(*Esublim, petday);
        ...
        if (*Esublim > snowtot) {
          ...
          *snlq = 0.0f;
          ...
          /* in proportion. */
          *snow *= (1.0f - *Esublim/snowtot);
          *snlq *= (1.0f - *Esublim/snowtot);
        }
...
**             with PILPS experimental runs. Net incoming radiation (solrad)
...
**    jday      - current julian day (1..366)
**    solrad    - net incoming solar radiation (langleys/day)
**                (changed from local name netrad for traceability KLK)
**    relhum    - relative humidity (%)
**    rlatitude - latitude of the site (in radians)
**    tavg      - average temperature for the day (deg C - 2m)
**    windsp    - average daily windspeed at 2 meters (mph)
...
*****************************************************************************/
    ...
    float pet_snow(int jday, float tavg, float windsp, float relhum,
                   float solrad, float Emelt, float albedo, float rlatitude,
                   float clrsky)
    {
      float  declin, ahou;
      float  shwave, kelvin, arads, ftemp;
      float  petfunc;
