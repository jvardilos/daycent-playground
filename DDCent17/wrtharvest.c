...
**                   event (g/m^2)
**    afertapp1    - amount of automatic nitrogen fertilizer applied since
**                   pervious HARV event (g/m^2)
**    afertapp2    - amount of automatic phosphorus fertilizer applied since
**                   pervious HARV event (g/m^2)
**    afertapp3    - amount of automatic sulfur fertilizer applied since
**                   pervious HARV event (g/m^2)
...
#include "soilwater.h"
    ...
    void wrtharvest(float *time, int *dayofyr, char crpval[11],
                    float *agcacc, float *bgcjacc, float *bgcmacc,
                    float *cgrain, float *egrain1, float *egrain2, float *egrain3,
                    float *crmvst, float *ermvst1, float *ermvst2, float *ermvst3,
                    float *cstraw, float *estraw1, float *estraw2, float *estraw3,
                    float *stdstraw, float *estdstraw1, float *estdstraw2, float *estdstraw3,
                    float *addsdc,  float *addsde1, float *addsde2, float *addsde3,
                    float *resid,   float *reside1, float *reside2, float *reside3,
                    float *irrapp,  float *fertapp1, float *fertapp2, float *fertapp3,
                                    float *afertapp1, float *afertapp2, float *afertapp3,
                    ...
                    float *accrst,  float *accrste1, float *accrste2, float *accrste3,
                    float *ctubesj, float *etubesj1, float *etubesj2, float *etubesj3,
                    float *ctubesm, float *etubesm1, float *etubesm2, float *etubesm3,
                    float *srfclittrj, float *esrfclittrj1, float *esrfclittrj2, float *esrfclittrj3,
                    float *soillittrj, float *esoillittrj1, float *esoillittrj2, float *esoillittrj3,
                    float *srfclittrm, float *esrfclittrm1, float *esrfclittrm2, float *esrfclittrm3,
                    float *soillittrm, float *esoillittrm1, float *esoillittrm2, float *esoillittrm3)
    {
      ...
      fprintf(files->fp_harv, "%.2f,%d,%s,", *time, *dayofyr, crpval);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,", *agcacc, *bgcjacc, *bgcmacc);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,",
              *cgrain, *egrain1, *egrain2, *egrain3,
              *crmvst, *ermvst1, *ermvst2, *ermvst3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
              *cstraw,  *estraw1, *estraw2, *estraw3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
              *stdstraw, *estdstraw1, *estdstraw2, *estdstraw3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
              *addsdc, *addsde1, *addsde2, *addsde3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
              *resid, *reside1, *reside2, *reside3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,", *irrapp,
              *fertapp1, *fertapp2, *fertapp3,
              *afertapp1, *afertapp2, *afertapp3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
      ...
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,",
              *accrst, *accrste1, *accrste2, *accrste3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,",
              *ctubesj, *etubesj1, *etubesj2, *etubesj3,
              *ctubesm, *etubesm1, *etubesm2, *etubesm3);
      fprintf(files->fp_harv, "%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,",
